...
const citiesList = [
  ...
  'Brooklyn', 'Manhattan', 'Long Island', 'New Jersey', 'Atlantic City', 'Columbus, Oh',
  ...
  // Австрия
  'Vienna', 'Wien', 'Graz', 'Linz', 'Salzburg', 'Innsbruck', 'Klagenfurt', 'Villach', 'Wels', 'Sankt Pölten', 'Dornbirn',
  'Steyr', 'Wiener Neustadt', 'Feldkirch', 'Bregenz', 'Leonding', 'Klosterneuburg', 'Baden', 'Wolfsberg', 'Leoben', 'Krems',

  // Швеция
  'Stockholm', 'Gothenburg', 'Goteborg', 'Malmö', 'Malmo', 'Uppsala', 'Västerås', 'Örebro', 'Linköping', 'Helsingborg', 'Jönköping', 'Norrköping',
...
// Validate city function (из Next.js версии)
export const isCityValid = (city) => {
  if (!city || typeof city !== 'string') return false
  
  const normalizedCity = city.trim()
  if (normalizedCity.length < 2) return false
  
  // Check if city exists in cities array
  return cities.some(cityItem => 
    cityItem.toLowerCase() === normalizedCity.toLowerCase()
  )
}

// Get city suggestions for empty input (из Next.js версии)
export const getCitySuggestions = (limit = 8) => {
  return popularCities.slice(0, limit)
}
