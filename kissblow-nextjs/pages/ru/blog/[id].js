...
const BlogPost = ({ post, relatedPosts, translations, lastUpdated }) => {
  ...
  const seoData = {
    ...
    url: `https://kissblow.me/ru/blog/${post.id}`,
    canonical: `https://kissblow.me/ru/blog/${post.id}`,
    alternate: {
      'en': `https://kissblow.me/blog/${post.id}`
    },
    type: 'article',
  ...
  const breadcrumbs = [
    ...
    { name: post.title, path: `/ru/blog/${post.id}` }
  ]
          ...
          <article className="theme-surface rounded-lg border theme-border">
            ...
            <div className="p-8">
              ...
              <div className="flex flex-wrap items-center gap-4 mb-6 text-sm theme-text-secondary">
                ...
                <div className="flex items-center space-x-2">
                  <Calendar size={16} />
                  <span>{t('blog.publishedOn')}: {new Date(post.date).toLocaleDateString('ru-RU')}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Clock size={16} />
